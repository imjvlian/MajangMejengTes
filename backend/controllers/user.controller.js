...
export const signout = async (req, res, next) => {
  try {
    res.clearCookie("acces_token").status(200).json("Logged out!");
  } catch (error) {
    next(error);
  }
};
...
export const getUsers = async (req, res, next) => {
  ...
  try {
    ...
    const users = await User.find()
      .sort({ isAdmin: 1, createdAt: sortDirection })
      .skip(startIndex)
      .limit(limit);
...
export const toggleAdmin = async (req, res, next) => {
  ...
  try {
    ...
    if (user._id.toString() === req.user.id) {
      return next(errorHandler(400, "You can't change your own admin status"));
    }
